...
# Necessaria per num2string - max grandezza 4 miliardi = 10 posti
tempString: .ascii "          \0"

# Ritorna la stringa in caso di risultato invalido
invalidString: .ascii "Invalid\0"
...
pilotFound: .byte 0 # ? INUTILE, basta controllare la somma
...
movl 16(%esp), %esi
movl 20(%esp), %eax # Contiene l'indirizzo della stringa in outputte
movl %eax, outputString
movl outputString, %eax
...
popl %eax
popl %esi
# ? Inutile jmp continue
continue: 
incb %dh
# ?? asd pushl %eax # ? MANCA POP?
# ?? asd pushl %ebx # ? MANCA POP?
...
movl outputString, %edi
movl outputString_offset, %ecx

movl totalRows, %eax
cmpl $0, %eax
jne notInvalid
leal invalidString, %esi
invalidLoop:
movb (%esi, %ecx), %dl
cmpb $0, %dl
je invalidEnd
...
jmp avgSpeedLoop
endAvgSpeedLoop:
movl %ecx, outputString_offset

leal outputString, %eax
